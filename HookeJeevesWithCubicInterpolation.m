function [x1, y1, x2, y2, x3, y3] = HookeJeevesWithCubicInterpolation(fun, x0, y0, ax, bx, exp)
ay=ax;
by=bx;
q=0.1;
x3=x0;
...
while (exp >= q)
    ...
    if ((y2 - y0) > 0)
        if (abs(k) >= 1)
            ...
            g = @(X1) integral(@(X2) fun(X1,X2) , bx,k);
            x3=CubicInterpolation(g,x0, delta, eps1, eps2);
            %x3=CubicInterpolation(g,x0,y0,x0,bx,k,e);
            y3=k*(x3 - x0)+y0;
            k31=(y3 - y0)/sqrt((y3 - y0)*(y3 - y0)+(x3 - x0)*(x3 - x0));
        end
        ...
        if (abs(k) >= 1)
            g = @(X1) integral(@(X2) fun(X1,X2) , ay,y0);
            y3=CubicInterpolation(g,x0, delta, eps1, eps2);
            %y3=CubicInterpolation(g,x0,y0,ay,y0,k,e);
            x3=x0-(y0-y3)/k;
            k31=(y3 - y0)/sqrt((y3 - y0)*(y3 - y0)+(x3 - x0)*(x3 - x0));
        else
            g = @(X1) integral(@(X2) fun(X1,X2) , x0,k);
            x3=CubicInterpolation(g,x0, delta, eps1, eps2);
            %x3=CubicInterpolation(g,x0,y0,ax,x0,k,e);
            y3=y0-k*(x0-x3);
            k31=(y3 - y0)/sqrt((y3 - y0)*(y3 - y0)+(x3 - x0)*(x3 - x0));
        end
    end

function varargout = MultiParametricGUI(varargin)
gui_Singleton = 1;
...
% --- Executes just before MultiParametricGUI is made visible.
...
addColumnNames(handles, columnNames);
set(handles.table, 'Data', tableData);
set(handles.pushbuttonMinMaxMethod, 'Enable', 'on');
...
% --- Executes on button press in pushbuttonMinMaxMethod.
function pushbuttonMinMaxMethod_Callback(hObject, eventdata, handles)
tableData = get(handles.table, 'Data');
...
for j=1:numberOfFunctions
    ...
    if currLambda < 0 
        msgbox('Error! Lambda is negative');
        return;
    end
...
set(handles.pushbuttonMinMaxMethod, 'Enable', 'off');
...
%tableDataBinar = get(handles.TableBinar, 'Data');
...
tableDataB = getStartPoint(numberOfSolutions, numberOfParameters,numberOfFunctions,directionOfFunctions,handles);
set(handles.TableBinar,'Data', tableDataB);


function tableDataB = getStartPoint(numberOfSolutions, numberOfParameters,numberOfFunctions,directionOfFunctions,handles)
tableDataB = {''};
tableDataBest = [];
tableDataLow = [];
tableData = get(handles.table, 'Data');
...
for i=1:numberOfSolutions
    ...
    for k=1:numberOfFunctions
        ...
        if(i~=j)
        rowOne = tableData(i,numberOfParameters+k);
        rowTwo = tableData(j,numberOfParameters+k);
        one = double(cell2mat(rowOne));
        two = double(cell2mat(rowTwo));
        if(strcmp(directionOfFunctions(1,k),'DOWN'))
          if(one < two)
              Num1=Num1+1;
          end
          if(one > two)
        ...
        if(strcmp(directionOfFunctions(1,k),'UP'))
            if(one < two)
              Num2=Num2+1;
            end
            if(one > two)
      ...
      if(i ~=j)
        ...
        str=strcat(n1, ' : ', n3);
        tableDataB(i,j) = {str};
        tableDataBest(i,j) = Num1
        tableDataLow(i,j) = Num2
        Num1=0;
        Num2=0;
      end  
    end
...
for i=1:numberOfSolutions 
    sum = 0
    for j=1:numberOfSolutions
        sum = sum + tableDataBest(i,j)
    end
    tableDataB(i, numberOfSolutions + 1) = {sum}
end
column = getDataFromColumn(tableDataB(:, numberOfSolutions + 1), intmax('int32'));
indexInColumn = indexOfMax(column);
point = num2str(cell2mat(tableDataB(indexInColumn, numberOfSolutions + 1)));
tableDataB(indexInColumn,  numberOfSolutions + 1) = {markBest(point)};


function tableDataBinar = getPointsBinar(numberOfSolutions, numberOfParameters, handles, tableData)
tableDataBinar = [];
for i=1:numberOfParameters
    ...
    tableDataBinar = [tableDataBinar; newRow];
end


% --- Executes when entered data in editable cell(s) in TableBinar.
function TableBinar_CellEditCallback(hObject, eventdata, handles)
% hObject    handle to TableBinar (see GCBO)
